...
public class PomodoroSistema : MonoBehaviour
{
    ...
    public void UsarTempo(Tempos tempo)
    {
        ...
        // 🔔 Programar la notificación al finalizar el tempo
    ...
    private void VerificarSiTempoTerminado()
    {
        ...
        if (transcurrido >= duracion)
        {
            ...
            // Prepara el siguiente tempo (opcional, ajusta según lógica)
            // Aquí podrías forzar que se cargue el siguiente tempo
        }
        else
        {
            Debug.Log("Aún no termina el tempo, quedan " + (duracion - transcurrido).TotalSeconds + " segundos");
        }
    }
    ...
    {
        if (!pauseStatus) // volvió al frente
        {
            VerificarSiTempoTerminado();
        }
    }
